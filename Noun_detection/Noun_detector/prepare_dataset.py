...
def parse_args():
  ...
  parser.add_argument('--image_dir', dest='image_dir',
                      ...
                      default="VGG") # /left0100.jpg
  parser.add_argument('--save_dir', dest='save_dir',
                      help='directory to save results',
                      default="save") # C:/Users/TaoZ/TUM/HAU/hand_object_detector/
...
if __name__ == '__main__':
  ...
  with torch.no_grad():
    ...
    while (num_images > 0):
        ...
        # img = im_in
        im_file_crop = os.path.join('cropped_image', imglist[num_images])
        # cv2.imshow('orignal', im_in)
        # key = cv2.waitKey(0) # '0' means waiting forever
        # if key == ord('q'): # pressed escape
        #     cv2.destroyAllWindows()
        if obj_dets is not None:
            for i, item in enumerate(obj_dets):
                bbox = item[: 4].astype(int)
                
                cropped_image = im_in[bbox[1]:bbox[3], bbox[0]:bbox[2]]
                # cropped_image = img[736:1027, 258:363]
                # cv2.imshow("cropped", cropped_image)
                # key = cv2.waitKey(0) # '0' means waiting forever
                # if key == ord('q'): # pressed escape
                #     cv2.destroyAllWindows()
                crop_path = im_file_crop.split('.')[: -1][0] + "_" + str(i) + '.jpg'
