...
from new_crop_hand_obj_detection_detector import *
...
def hand_obj_one_solution(image_array):
    torch.cuda.empty_cache()
    absolute_path =  os.getcwd() + '/Noun_detection/Noun_detector'
    inference_per_frame = []
    fasterRCNN, pascal_classes = load_model()
    
    for i,image in enumerate(os.listdir(image_array)):
        path = os.path.join(image_array, image)
        ...
        im2show = one_soluition_hand_obj_detection_detector(image_ori, inference_per_frame, absolute_path, fasterRCNN, pascal_classes, image)
