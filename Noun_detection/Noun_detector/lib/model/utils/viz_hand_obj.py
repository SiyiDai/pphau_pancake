...
def draw_obj_mask(image, draw, obj_idx, obj_bbox, obj_score, width, height, font, obj_name):
    ...
    draw.rectangle([obj_bbox[0], max(0, obj_bbox[1]-30), obj_bbox[0]+len(obj_name)*20, max(0, obj_bbox[1]-30)+30], fill=(255, 255, 255), outline=obj_rgb, width=4)
    image_without_text = copy.deepcopy(image)
    draw.text((obj_bbox[0]+5, max(0, obj_bbox[1]-30)-2), obj_name, font=font, fill=(0,0,0)) #

    return image, image_without_text

def draw_obj_mask_simpler(image, draw, obj_bbox, width, height):
    ...
    draw.rectangle([obj_bbox[0], max(0, obj_bbox[1]-30), obj_bbox[0], max(0, obj_bbox[1]-30)+30], fill=(255, 255, 255), outline=obj_rgb, width=4)
