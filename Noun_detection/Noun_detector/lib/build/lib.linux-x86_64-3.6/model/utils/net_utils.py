...
def vis_detections_filtered_objects_PIL_classification(im, obj_dets, hand_dets, PATH, thresh_hand=0.8, thresh_obj=0.01, font_path='lib/model/utils/times_b.ttf'):
    sys.path.append(PATH)
    # convert to PIL
    im = im[:,:,::-1]
    image = Image.fromarray(im).convert("RGB")
        ...
        for obj_idx, i in enumerate(range(np.minimum(10, obj_dets.shape[0]))):
            ...
            if score > thresh_obj and i in img_obj_id:
                ...
                if cv2.waitKey(1000): #& 0xFF == ord('q'):
                    cv2.destroyAllWindows()
                print('###############using VGG###############')
                name, prob = efficient_predict(cropped_image, PATH)
            ...
            if len(obj_name)>0 and len(obj_bbox)>0:
                for n in range(len(obj_name)):
                    image = draw_obj_mask(image, draw, obj_idx, obj_bbox[n], score, width, height, font, obj_name[n])
    ...
    if (obj_dets is not None) and (hand_dets is not None):
        ...
        for obj_idx, i in enumerate(range(np.minimum(10, obj_dets.shape[0]))):
            ...
            if score > thresh_obj and i in img_obj_id:
                ...
                cropped_imagecv2 = cropped_image.copy()
                cropped_imagecv2 = np.array(cropped_imagecv2)
                filename = os.path.join(PATH, str(number)+'_'+image_name)
