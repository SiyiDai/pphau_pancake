...
from model.utils.net_utils import save_net, load_net, vis_detections, vis_detections_PIL, vis_detections_filtered_objects_PIL, vis_detections_filtered_objects, calculate_center # (1) here add a function to viz
...
# os.chdir('C:/Users/TaoZ/TUM/HAU/hand_object_detector')
...
def calculate_IoU(bbox, loaded_dict):
    ...
    for item in loaded_dict.keys():
        ...
        obj_bbox = [obj[0, 0], obj[0, 1], obj[0, 0] + obj[1, 0], obj[0, 1] + obj[1, 1]]
...
def one_soluition_hand_obj_detection_detector(image, loaded_dict, absolute_path, fasterRCNN, pascal_classes):
...
###############################################################################
      ...
      if obj_dets is not None:
          ...
          for i in range(len(obj_dets)):
              ...
              obj_name.append(calculate_IoU(bbox, loaded_dict))
...
###############################################################################
          if vis:
            # visualization
            im2show = vis_detections_filtered_objects_PIL(im2show, obj_dets, hand_dets, obj_name, absolute_path, thresh_hand, thresh_obj)
...
###############################################################################
              # folder_name = args.save_dir
              # os.makedirs(folder_name, exist_ok=True)
              # result_path = os.path.join(folder_name, image_name[:-4] + "_det.png")
              # im2show.save(result_path)
          else:
              im2showRGB = cv2.cvtColor(im2show, cv2.COLOR_BGR2RGB)
              cv2.imshow("frame", im2showRGB)
          ...
          if name_append is not None:
              obj_name.append(name_append)
              bbox_obj.append(bbox_p)
              print("the active objects are:", obj_name)
              # return obj_name, bbox_obj
              return im2show
          else:
              print("the active objects are:", obj_name)
              # return obj_name, bbox_obj
