...
def read_split_data(root: str, val_rate: float = 0.2, test_rate: float = 0.0):
    ...
    with open('/content/drive/MyDrive/HAU/efficientNet_tf/class_indices.json', 'w') as json_file:
    ...
    for cla in flower_class:
        ...
        for img_path in images:
            ...
            elif img_path in test_path:  # 如果该路径在采样的验证集样本中则存入验证集
...
def generate_ds_TVT(data_root: str,
                im_height: int,
                im_width: int,
                batch_size: int,
                val_rate: float = 0.1,
    ...
    :return:
    """
    train_img_path, train_img_label, val_img_path, val_img_label, test_img_path, test_img_label = read_split_data(data_root, val_rate=val_rate, test_rate=test_rate)
    AUTOTUNE = tf.data.experimental.AUTOTUNE
    ...
    test_ds = configure_for_performance(test_ds, total_test)
